...
const PROJECTS = [
  ...
  },
  {
    ...
    description:
      "Pose-estimation tool that grades a road bike fit from a single " +
      "phone video — no professional fitter, no pressure sensors, just a " +
      "trainer and a side-on clip.",
    steps: [
      {
        title: "The Problem",
        description:
          "A professional bike fit checks a handful of joint angles at " +
          "the bottom of the pedal stroke — knee flexion, torso angle, " +
          "elbow bend, shoulder angle — against ranges backed by " +
          "sport-science research. Doing this yourself from a video is " +
          "normally guesswork: you're eyeballing angles frame-by-frame " +
          "with no ground truth to check against.",
      },
      {
        title: "What It Does",
        description:
          "Runs YOLO11 pose estimation on every frame of a pedaling " +
          "video, extracting 17 body keypoints per frame with confidence " +
          "scores. It automatically finds bottom-dead-center (BDC) of the " +
          "pedal stroke, computes the knee, torso, elbow, shoulder, and " +
          "hip angles from those keypoints, grades each one green / amber " +
          "/ red against published bike-fit research ranges, and outputs " +
          "a plain-English fix for anything that's off.",
      },
      {
        title: "The Visuals",
        video: "images/bike-fit-overlay.mp4",
        description:
          "The tool draws a color-coded skeleton directly onto the " +
          "original video, frame by frame, so you can see exactly which " +
          "angle it measured and where — not just a final number with no " +
          "way to check it.",
      },
      {
        title: "How It Works",
        ...
        description:
          "Stack: Ultralytics YOLO11 (the large 'x' pose variant — the " +
          "small one loses knee/ankle keypoints through motion blur) for " +
          "keypoint detection, OpenCV + NumPy for the angle geometry, " +
          "Roboflow supervision for video annotation, PyTorch underneath, " +
          "ffmpeg for H.264 re-encoding. The hard part was finding BDC " +
          "reliably — knee angle is noisy frame-to-frame, so instead of " +
          "trusting one frame, the tool treats knee angle over time as a " +
          "signal, finds local maxima with a minimum spacing constraint " +
          "so it can't double-count a stroke, then takes the median angle " +
          "across every detected stroke — 45 of them in this run — which " +
          "is what makes the grade trustworthy instead of relying on one " +
          "lucky screenshot.",
      },
      {
        title: "Results",
        description:
          "Ran it on my own fit: knee flexion (39°), torso angle (47°), " +
          "and shoulder angle (86°) all graded green against their " +
          "30-40°, 40-50°, and 80-95° targets — saddle height and reach " +
          "were already close to right. Elbow angle graded red at about " +
          "4°, essentially locked out against a 15-30° target, flagging a " +
          "reach/cockpit adjustment I wouldn't have caught by feel. Hip " +
          "angle came back amber at 113° against an 85-110° target.",
      },
    ],
  },
